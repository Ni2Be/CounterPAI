...
Eval::NN::NN(int64_t input, int64_t hidden, int64_t output, int64_t hidden_layer_count, Opti opti, Loss_F loss_f, double dropout, torch::Device device)
	:
	device(device),
	s_optimizer(opti),
...
{
	if (dropout > 0)
		std::cout << "\ndropout for Linear NN not supportet!\n";
...
void Eval::NN::set_learning_rate(double learning_rate)
{
...
torch::Tensor Eval::NN::forward(torch::Tensor x)
{
...
torch::Tensor Eval::NN::learn_step(const torch::Tensor& learn_data, torch::Tensor target_data, bool optimize)
{
...
torch::Tensor Eval::NN::test_prediction(torch::Tensor test_data, torch::Tensor target_data)
{
	return learn_step(test_data, target_data, false);
}

std::shared_ptr<torch::optim::Optimizer> Eval::NN::get_optimizer()
{
	return optimizer;
}