...
Eval::Data_Loader::Data_Loader(Learn_Settings settings, bool is_training)
	:
	settings(settings)
{
	if (!is_training)
	...
	std::vector<Sheet_Music> test_sheets;
	test_sheets = load_sheets(settings.test_data_folder);
	...
	{
		evaluate_fux_rules_from_two_sides_lnn_1(sheets, train_features_vec, train_targets_vec);
		evaluate_fux_rules_from_two_sides_lnn_1(test_sheets, test_features_vec, test_targets_vec);
	}
...
std::tuple<torch::Tensor, torch::Tensor> Eval::Data_Loader::get_train_batch()
{
	return get_batch(train_features_vec, train_targets_vec);
}

std::tuple<torch::Tensor, torch::Tensor> Eval::Data_Loader::get_test_batch()
{
...
std::vector<std::vector<float>> Eval::Data_Loader::convert_to_2d_vector(Sheet_Music& sheet)
{
	...
	*/
	...
	for (int sixteenths = 0; sixteenths < eights_count * 2; sixteenths += 2)
	{
		...
		if (cp_was_at_note)
		{
			Rule_Evaluation temp(cp_note.get_note_info(Rule_Evaluation::C_INDEX_NAME));
